...
// --- PlatformIO export (Prioriti 1.3): zip berisi platformio.ini + src/main.cpp + libraries.txt ---
const PIO_ENVS = {
    ard5:    { platform: 'atmelavr',       board: 'uno',          name: 'Arduino Uno' },
    ard33:   { platform: 'atmelavr',       board: 'nanoatmega328', name: 'Arduino Nano' },
    esp32:   { platform: 'espressif32',    board: 'esp32dev',     name: 'ESP32 DevKit' },
    esp8266: { platform: 'espressif8266',  board: 'nodemcuv2',    name: 'NodeMCU ESP8266' },
    pico:    { platform: 'raspberrypi',    board: 'pico',         name: 'RP2040 (Pico)' }
};
function getLibraries() {
    ...
    helperComps.forEach(n => {
        ...
        if (c.k === 'dht') { libs.add('DHT sensor library'); libs.add('Adafruit Unified Sensor'); }
        if (/oled|ssd1306|sh1106/.test(l)) { libs.add('Adafruit SSD1306'); libs.add('Adafruit GFX Library'); }
        if (/lcd 16x2|lcd 20x4/.test(l)) libs.add('LiquidCrystal I2C');
        if (l.includes('rc522')) libs.add('MFRC522');
        if (l.includes('nrf24')) libs.add('RF24');
        if (/mpu-6050|mpu-9250/.test(l)) libs.add('MPU6050');
        if (/bme280|bmp280/.test(l)) libs.add('Adafruit BME280 Library');
        if (c.k === 'servo' && famKey !== 'ard5' && famKey !== 'ard33') libs.add('ESP32Servo');
    });
    return [...libs];
}
function exportPlatformIO() {
    const code = document.getElementById('code-block').textContent;
    if (!code.trim() || code.startsWith('// No active')) return showToast('Generate code first');
    const famKey = MCU_INDEX[helperMcu] ? MCU_INDEX[helperMcu].family : '';
    const env = PIO_ENVS[famKey];
    if (!env) return showToast('PlatformIO export supports Arduino-framework boards (Uno/Nano/ESP32/ESP8266/Pico)');
    if (typeof JSZip === 'undefined') return showToast('Zip library still loading — try again in a second');

    const cp = getCodeParams();
    const libs = getLibraries();
    let ini = '; PlatformIO project — generated by Pieware 2\n; Open this folder in VS Code + PlatformIO extension, then Upload\n';
    ini += '[env:' + env.board + ']\n';
    ini += 'platform = ' + env.platform + '\n';
    ini += 'board = ' + env.board + '\n';
    ini += 'framework = arduino\n';
    ini += 'monitor_speed = ' + cp.baud + '\n';
    if (libs.length) ini += 'lib_deps =\n' + libs.map(l => '    ' + l).join('\n') + '\n';

    let src = '#include <Arduino.h>\n\n' + code;
    // .ino → .cpp: deklarasi forward untuk fungsi Arduino (setup/loop dah ada; helper fns tiada di sini)
    const libsTxt = 'Third-party libraries required (auto-detected from your components):\n' +
        (libs.length ? libs.map(l => '- ' + l).join('\n') : '- None (core libraries only)') +
        '\n\nIn Arduino IDE: Tools > Manage Libraries and search by name.\n';

    const zip = new JSZip();
    zip.file('platformio.ini', ini);
    zip.file('src/main.cpp', src);
    zip.file('libraries.txt', libsTxt);
    zip.generateAsync({ type: 'blob' }).then(blob => {
        const url = URL.createObjectURL(blob);
        const a = document.createElement('a');
        a.href = url;
        a.download = 'pieware2_platformio_' + env.board + '.zip';
        a.click();
